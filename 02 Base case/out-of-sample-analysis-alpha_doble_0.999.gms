...
LOOP(t,IF(mpDAfix(t) eq 0,mpDAfix(t) = 1e-6));
DISPLAY mpDAfix;

LOOP(s$S3(s),LOOP(xi$X3(xi),LOOP(itAlpha,IF(ProfitScen(s,xi,itAlpha) eq 0,ProfitScen(s,xi,itAlpha) = 1e-6))));
DISPLAY ProfitScen;

LOOP(itAlpha,IF(AlphaCVARvector(itAlpha) eq 0,AlphaCVARvector(itAlpha) = 1e-6));
DISPLAY AlphaCVARvector;

execute_unload 'results_OA_doble_0.999.gdx' mpDAfix, ProfitScen, AlphaCVARvector;
