%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Paper Zagreb 2025
% Analysis battery and electrolyzer - Previous model vs Proposed model
% Plot Figure 9
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
data(1,1,:) = [IBbatExpectedPrevious(indPrev(1)) IBeleExpectedPrevious(indPrev(1))];
data(1,2,:) = [IBbatExpectedProposed(indProp(1)) IBeleExpectedProposed(indProp(1))];

data(2,1,:) = [IBbatExpectedPrevious(indPrev(2)) IBeleExpectedPrevious(indPrev(2))];
data(2,2,:) = [IBbatExpectedProposed(indProp(2)) IBeleExpectedProposed(indProp(2))];

data(3,1,:) = [IBbatExpectedPrevious(indPrev(3)) IBeleExpectedPrevious(indPrev(3))];
data(3,2,:) = [IBbatExpectedProposed(indProp(3)) IBeleExpectedProposed(indProp(3))];

color_bateria        = [0.07 0.44 0.75];  
color_electrolizador = [0.87 0.33 0];     

h = gobjects(2,2);

for i = 1:2
    x_pos = x + (i - 1.5)*(width + gap);
    y = squeeze(data(:,i,:));

    b = bar(x_pos, y, width, 'stacked');

    b(1).FaceColor = color_bateria;
    b(2).FaceColor = color_electrolizador;

    h(i,:) = b;

    if i == 1
        set(b, 'EdgeColor', 'k', 'LineStyle', '-')   
    else
        set(b, 'EdgeColor', 'k', 'LineStyle', '--')  
    end
end
...
xlim([0.5 3.5])
xticks([1 2 3])
xticklabels({'Optimistic','Intermediate','Pessimistic'})

set(gca,'TickLabelInterpreter','latex');
set(gca,'FontSize',14);
xlabel('Case','Interpreter','tex','FontSize',16,'FontName','Times New Roman');
ylabel('Power (MW)','Interpreter','tex','FontSize',16,'FontName','Times New Roman');

box on

hModel1 = plot(nan, nan, 'k-');
hModel2 = plot(nan, nan, 'k--');

legend([h(1,1), h(1,2), hModel1, hModel2], ...
       {'Battery', 'Electrolyzer', 'Previous model (left)', 'Proposed model (right)'}, ...
